  ...
  return (
    <div className="relative">
      ...
      <div className="fixed top-4 left-0 right-0 flex justify-center z-50">
        <AnimatePresence>
          {notification && (
            <motion.div className="bg-techno-turquoise text-white text-center mt-10 text-sm p-2 px-4 w-[90%] max-w-[300px] rounded-[5px]" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              {notification}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="fixed top-16 left-0 right-0 flex justify-center z-50">
        <AnimatePresence>
          {error && (
            <motion.div className="bg-red-600 text-white text-center text-sm p-2 px-4 w-[90%] max-w-[300px] rounded-[5px]" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              {error}
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="bg-cover bg-no-repeat h-screen w-screen flex" style={{ backgroundImage: `url(${background})` }}>
        <DashboardElement />
        <div className={`bg-white bg-opacity-5 w-[90%] justify-center max-w-[600px] h-[550px] m-auto rounded-[35px] backdrop-blur-[10px] flex flex-col text-white ${isScreenSmall ? "" : "ml-[600px]"}`}>
          {!user ? (
            <div className="flex flex-col justify-center items-center h-full mt-10">
              <motion.p className="text-xl md:text-2xl text-white font-bold text-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            ...
            <>
              ...
              <form onSubmit={(e) => e.preventDefault()} className="flex flex-col items-center mt-10">
                <AnimatePresence>
                  <motion.div className="mb-6 relative w-[90%] max-w-[450px]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ delay: 0.1 }}>
                    <input
                      id="name"
                      type="text"
                      value={name}
                  ...
                  <motion.div className="mb-6 relative w-[90%] max-w-[450px]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ delay: 0.2 }}>
                    <input
                      id="division"
                      type="text"
                      value={division}
                  ...
                  <motion.div className="mb-6 relative w-[90%] max-w-[450px]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ delay: 0.3 }}>
                    <input
                      id="salary"
                      type="text"
                      value={salary}
